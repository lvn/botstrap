...
var Bot = function Bot(options) {
  ...
  this.baseContext = {
    bot: this,
    client: this.rtm,
    channels: this.rtm.channels,
    dms: this.rtm.dms,
    groups: this.rtm.groups,
    users: this.rtm.users,
    bots: this.rtm.bots,
...
Bot.prototype.on = function on(type, cb, config) {
  ...
  this.rtm.on(type, function() {
    ...
    if (context.message) {
      ...
      context.channel = Object.assign({},
        _this.rtm.dataStore.channels[context.message.channel], {
          send: (msg) => {
            _this.rtm.sendMessage(msg, context.message.channel);
          }
        });
      context.user = _this.rtm.dataStore.users[context.message.user];

      // TODO: don't construct response inside this if-block.
      response = new Response(_this.web, context.channel, context.message);
    }
