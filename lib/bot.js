...
var Bot = function Bot(options) {
  ...
  this.on('message', this._dispatchCommand);
};
...
Bot.prototype._dispatchCommand = function _dispatchCommand(context, message) {
  ...
  context.config = this.configs[words[0]];
  this.commands[words[0]] &&
    introject.injectDeps(this.commands[words[0]], context)();
};
